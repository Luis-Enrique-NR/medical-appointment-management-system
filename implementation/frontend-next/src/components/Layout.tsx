...
import {
  ...
  Stethoscope, Bell, LogOut, CalendarCheck, CheckCircle, Info, XCircle, Menu, X, History,
} from "lucide-react";
import { CliniferLogo } from "./CliniferLogo";
import type { Role, Screen } from "@/lib/types";

interface NavItem { id: Screen; label: string; icon: React.ReactNode }
...
const navItems: Record<Role, NavItem[]> = {
  ...
  doctor: [
    ...
    { id: "availability-history", label: "Historial de Propuestas", icon: <History size={18} /> },
    { id: "profile", label: "Mi Perfil", icon: <User size={18} /> },
  ],
};
...
interface Notification {
  id: number; type: "appointment" | "info" | "warning";
...
export function Layout({ role, userName, currentScreen, onNavigate, onLogout, children }: LayoutProps) {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    ...
    { id: 4, type: "info", message: "Recordatorio: Tiene una cita mañana a las 09:00 AM.", time: "Hace 1 hora", read: false },
  ]);
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-[#05576D] flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex flex-col items-center px-6 py-6 border-b border-white/10">
          <CliniferLogo height={40} inverted />
          <span className={`mt-3 px-3 py-0.5 rounded-full text-xs text-white ${roleColors[role]}`}>{roleLabels[role]}</span>
        </div>
          ...
          {items.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { onNavigate(item.id); setSidebarOpen(false); }}
                className={`w-full flex items-center gap-3 px-5 py-3 text-sm transition-colors ${active ? "bg-[#006FC1] text-white" : "text-white/70 hover:text-white hover:bg-white/10"}`}>
                {item.icon}
        ...
        <header className="h-[60px] bg-white border-b border-gray-200 flex items-center px-4 lg:px-6 gap-4 flex-shrink-0">
          ...
          <div className="relative">
            <button onClick={() => setNotifOpen(!notifOpen)}
              ...
              <Bell size={20} />
              {unread > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-[#FF82B6] text-white text-[10px] rounded-full flex items-center justify-center">{unread}</span>
              )}
            </button>
            {notifOpen && (
              <div className="absolute right-0 top-12 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                  <h3 className="font-semibold text-[#05576D] text-sm">Notificaciones</h3>
                  <div className="flex items-center gap-2">
                    {unread > 0 && (
                      <button onClick={() => setNotifications(prev => prev.map(n => ({ ...n, read: true })))}
                        className="text-xs text-[#006FC1] hover:underline">Marcar leídas</button>
                    )}
